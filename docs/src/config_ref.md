...
| `key` | `array of string` | yes | Public key(s) of this user. Can be given multiple times. |
...
| `name` | `string (non-empty)` | no | Name of the secret. Defaults to the path when omitted. |
...
| `rotate` | `array` | no | Rotation steps replacing this secret with a fresh one. Not implemented yet, see the Rotation page. |
| `swap` | `array of object` | no | Commands propagating a rotated secret to where it is used. Not implemented yet, see the Rotation page. |
