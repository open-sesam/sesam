...
`ssh` itself is meant for networking, we were only interested in the key
concept. Luckily `age` exists, which do not support only `ssh` keys for hybrid
encryption but also gives us a nice encryption format and post quantum keys.
...
The encoding of the audit log. TODO: link to explanation of audit log.
...
Each user possesses a Ed25519 signing key, generated during `sesam init`` or
...
- Hybrid encryption using `age`` (using X25519) and additional signature.
...
- Secrets are linked to audit log by storing a root hash, build out of individual hashes.
...
Using hybrid encryption only the recipients defined at encryption time is
...
Full integrity check is being done on revealing the secrets or when doing `sesam verify --fsck`.
...
something suspicious. As a user you can choose between keeping revealing (choosing more availability using just `git pull + sesam smudge filter`)
and high integrity (additionally running `sesam verify` or using `sesam open --pull`).
...
The audit log is append-only and cannot be replaced or truncated.
Replacement would be detected directly by noticing that the init uuid + signature changed.
Truncation can be detected by verifying that the audit log in the previous commit is a subset of the current one (i.e. it never "shrinks").
...
Given the same inputs, `sesam seal` will produce different cipher text. This is
on purpose and a design decision of `age`. This makes figuring out which secret
was changed harder for an attacker having read access.
...
- Some local operations (like viewing diffs or doing an checkout of old branches) do not require full verification on every step. We assume the user
  regularly runs `sesam verify --full` (e.g. during CI/CD) before doing anything serious like deployment.
...
- Limit the number of keys a user might have to 2 to avoid loosing overview.
...
- Make seal atomic (or at least close to) - interrupting it might lead to a failed root hash check. (TODO: Implement)
