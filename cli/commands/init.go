...
func HandleInit(ctx context.Context, cmd *cli.Command) error {
	// TODO: Look for sesam.yml or .sesam and take this as repo path and cancel of this exists
	...
	// TODO: at the end of init exec git add in sesam dir
	// TODO: add the admin to admin group
	// TODO: create a readme file where next steps are explained
	return nil
}
...
func resolveInitialRecipient(ctx context.Context, cmd *cli.Command, repoRoot, initialUser string) (age.Recipient, string, error) {
	recipientsArg := cmd.StringSlice("recipient")

	var recipients core.Recipients
	for _, recipientArg := range recipientsArg {
	...
	// TODO: move this to core as fallblack in there is no public key in for example in github
	for _, identity := range identities {
		switch i := identity.Identity.(type) {
...
// TODO: user core.recipient struc
...
func ensureDefaultGitIgnore(repoRoot string) error {
	...
	if _, statErr := os.Stat(gitignorePath); statErr == nil {
		fd, err = os.OpenFile(gitignorePath, os.O_APPEND|os.O_WRONLY, 0o600)
	} else if os.IsNotExist(statErr) {
		fd, err = os.Create(gitignorePath)
	} else {
		return fmt.Errorf("failed to access gitignore file: %w", statErr)
	}
	...
	if _, err := fd.Write([]byte(gitignoreContent)); err != nil {
		return fmt.Errorf("failed to create .gitignore at %s: %w", gitignorePath, err)
	}
	return nil
}

// TODO: make this like gitignore
...
func ensureVerifyHook(repoRoot string) error {
	...
	if err := renameio.WriteFile(hookPath, []byte(script), 0o755); err != nil {
...
// TODO: if there is no .git is not there, then break init and ask the user to create/generate .git
